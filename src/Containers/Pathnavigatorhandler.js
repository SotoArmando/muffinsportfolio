  ...
  return [
    ...
    <div className={`fixed corebox_3 left top back_2 boxshadow_24 z_index0 onactive_1 ${activebackground ? 'active' : ''}`}>
      <div className="modal gbasis_40 row allsize z_index4 novisible">
        ...
        <div className="row gbasis_36">
          {social.map((e, i) => <a href={`#${e}`} className={`center btn_u corebox_3 f_1 f_m_0 ffam_karla ${i === activepath ? ' f700 fore_blue_o0' : ' f600'} capitalize`}>{`${i === activepath ? '[:' : ''}${e}${i === activepath ? ']' : ''}`}</a>)}
        </div>
      </div>
    </div>,
    <div className={`${gridclassname || 'row  corebox_3 nav  center wrap   ls_30 '} `}>
      ...
      <div className="modal gbasis_40 row allsize z_index4">
        ...
        <div className="row gbasis_36">
          {social.map((e, i) => <a onClick={() => handlePathsClick(i)} href={`#${e}`} className={`center btn_u corebox_3 f_1 f_m_0 ffam_karla ${i === activepath ? ' f700 fore_blue_o0' : ' f600'} capitalize`}>{`${i === activepath ? '[:' : ''}${e}${i === activepath ? ']' : ''}`}</a>)}
        </div>
      </div>
    </div>,
