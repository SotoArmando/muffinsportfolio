...
export default function Homeprojectmodal({
  active, setModalActive, picturesprev, urls: [liveurl, giturl, tags, description],
}) {
  const [picture, setPicture] = useState(0);
  const modalReference = useRef();
  const landscapeElement = ({ i, marginv, marginh }) => (
    <div onClick={() => setPicture(i)} className={`pad_24 btn_b back_black  half_horizontalmar half_verticalmar mar_t${marginv} mar_b${marginv} mar_r${marginh} mar_l${marginh}`}>
      <div className={`pic_${picturesprev}_${i} corebox_7  corebox_x4  contain transparent `} />
    </div>
  );

  const labelElement = ({ _id: text, marginh, marginv }) => (
    <span className={`corebox_2 maxedcorebox_x7 back_blue_o0 fore_9 pad_l33 pad_r33 center half_horizontalmar half_verticalmar f600 mar_t${marginv} mar_b${marginv} mar_r${marginh} mar_l${marginh} ffam_karla tcenter`}>
      {text}
    </span>
  );

  useEffect(() => {
    const { current } = modalReference;
    if (current) {
      if (active) {
        console.log(active);
        current.focus();
  ...
  return (
    <div ref={modalReference} className={`fixed scrollable back_2 col left top allsize bodyheigth space_between items_center onactive_0 ${active ? 'active' : ''}`}>
      <div className="col modal">
        <div className="corebox_3 mobilecorebox_7" />
        <div className=" back_black ">
          <div className={`col corebox_16 mobilecorebox_14 pic_${picturesprev}_${picture} contain transparent`} />
        </div>
